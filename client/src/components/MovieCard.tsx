...
const MovieCard: React.FC<MovieCardProps> = ({ movie }) => {
    ...
    return (
        <div 
            className="min-w-[200px] max-w-[200px] flex flex-col items-center pb-2"
        >
            <div 
            ...
            <div className="w-full text-center">
                <h2 className="text-lg text-white font-bold mt-2 hover:text-blue-500 cursor-pointer inline" onClick={handleCardClick}>{movie.title}</h2>
            </div>
            <p className="text-sm my-2 text-white">Rating: {rating}</p>
        </div>
    );
};
